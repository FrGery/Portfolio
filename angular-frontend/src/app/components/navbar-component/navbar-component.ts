...
export class NavbarComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngOnInit(): void {
    this.sub = this.router.events.pipe(
      ...
      startWith(null),
      map(() => {
        // deepest active route data
        let r = this.route.firstChild ?? this.route;
        while (r.firstChild) r = r.firstChild;
        return {
          theme: r.snapshot.data['navbarTheme'] as string | undefined,
          url: this.router.url
        };
      })
    ).subscribe(({ theme, url }) => {
      ...
      // If AOS was booted, refresh on each nav end so new nodes animate
  ...
  // ordered delays (used by template data attributes)
